...
def find_transcripts(here):
    return sorted(here.glob(f"*{TRANSCRIPT_SUFFIX}"))
...
def main():
    ...
    transcripts = find_transcripts(here)

    if not transcripts:
        print(f"ERROR: no se encontro ningun archivo *{TRANSCRIPT_SUFFIX} en {here}")
        sys.exit(1)

    results = [parse_log(p) for p in transcripts]
    ...
    for r in results:
        if r["errors"] is None or r["warnings"] is None:
            print(f"ERROR: no se pudo parsear {r['file']} (formato inesperado)")
            sys.exit(1)
    ...
    print(f"Testbenches detectados: {[r['file'] for r in results]}")
